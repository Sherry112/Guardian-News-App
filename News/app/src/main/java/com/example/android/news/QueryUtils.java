...
public final class QueryUtils {
    ...
    private static ArrayList<News> extractFeatureFromJson(String newsJSON) {
        ...
        try {
            ...
            JSONObject result = baseJSONresponse.getJSONObject("response");
            JSONArray newsArray = result.getJSONArray("results");
            for (int i = 0; i < newsArray.length(); i++) {
                Bitmap thumbnailBitmap = null;


                JSONObject currentArticle = newsArray.getJSONObject(i);
                String sectionHeading = currentArticle.getString("sectionName");
                String title = currentArticle.getString("webTitle");
                String url = currentArticle.getString("webUrl");
                String publishTIme = currentArticle.getString("webPublicationDate");

                JSONObject additionalFields = currentArticle.getJSONObject("fields");
                if (additionalFields.has("thumbnail")) {
                    String image = additionalFields.optString("thumbnail");
                    URL imageUrl = new URL(image);
                    thumbnailBitmap = BitmapFactory.decodeStream(imageUrl.openConnection().getInputStream());

                }


                News article = new News(sectionHeading, publishTIme, title, url, thumbnailBitmap);
    ...
    //Help was taken to download thumbnail images from a StackOverflow Answer
